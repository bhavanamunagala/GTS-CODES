...
class Login extends Component {
  ...
  validateEmail = e =>   {
    ...
    validatePassword = e =>   {
      ...
      componentDidMount(){
        axios.get(`http://localhost:4710/api/v1/users/personal_details/${this.state.gts_user_id}`)
          .then(res=> {
            alert(res.data);
            this.setState({jobsString:this.getJobArrayTypo(res.data)});
            const personal_details=res.data;
            this.setState({
              personal_details,
                    });    
           alert(this.state.personal_details);
        },
       ...
       componentDidMountRole(){
        axios.get('http://localhost:4712/api/v1/users/role', {
          params: {
  ...
  submitUserLoginForm(e) {
    ...
    if (validationResult) {
        let fields = this.state.fields;
       // var url='http://ec2-13-232-50-184.ap-south-1.compute.amazonaws.com:4711/api/v1/users/login';
       // var url =`https://29c1fd20-e432-4d6a-98f0-ba273f4ab229.mock.pstmn.io/api/v1/users/login/success`;
        var url=`http://localhost:4711/api/v1/users/login/${fields["email"]}/${fields["password"]}`;
             ...
             axios.put(url, payload)
               .then(res => {
                 ...
                 if (status_code ===200) {
                   ...
                   ls.set('user_id',dataResponse.gts_user_id);
                   ls.set('personal_details', this.state.personal_details);
                   ls.set('roles', this.state.roles);
                   var ui = ls.getItem("user_id");
                   var pd = ls.getItem("personal_details");
                   var r = ls.getItem("roles");
                  console.log(ui)
                  console.log(pd)
                  console.log(r)
                   this.setState({
                  ...
                  if(fields["email"]=="admin@gmail.com"){
                   this.props.history.push('/AdminProfile');
                  }
                  else if(fields["email"]=="employer@gmail.com"){
                    this.props.history.push('/EmployerProfilePage');
                   }
                  else if(fields["email"]=="Jobseeker@gmail.com"){
                    this.props.history.push('/JobseekerProfile');
                   }  
                   else if(fields["email"]=="recruiter@gmail.com"){
                    this.props.history.push('/RecruiterProfile');
                   }
                   else if(fields["email"]=="trainer@gmail.com"){
