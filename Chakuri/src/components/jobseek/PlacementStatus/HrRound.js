...
class HrRound extends Component {
    constructor() {
        super();
        this.state = {
    ...
    componentDidMount(){
        ...
        .then((result) => {
            ...
            let filteredJob=this.state.totalJobs.filter(fj=>fj.gts_job_application_status=="HR round");
            this.setState({HrJob:filteredJob})
            console.log("HR Round Status are ",this.state.HrJob);
        }).catch((err) => {
            console.log("error");
        });
    }
    ...
    render() {
        const {error,HrJob}=this.state;
        console.log("in jobs",HrJob);
 

        return (this.state.HrJob.map(appliedJob=>(
