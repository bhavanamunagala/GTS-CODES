...
class Pending extends Component {
    ...
    componentDidMount(){
        ...
        .then((result) => {
            ...
            let filteredJob=this.state.totalJobs.filter(fj=>fj.gts_job_application_status=="Pending");
            this.setState({pendingJob:filteredJob})
            console.log("HR Round Status are ",this.state.pendingJob);
        }).catch((err) => {
            console.log("error");
        });
    }
    ...
    render() {
        const {error,pendingJob}=this.state;
        console.log("in jobs",pendingJob);
 

        return (this.state.pendingJob.map(appliedJob=>(
