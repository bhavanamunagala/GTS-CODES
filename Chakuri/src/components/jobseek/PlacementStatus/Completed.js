...
class Completed extends Component {
    ...
    componentDidMount(){
        ...
        .then((result) => {
            ...
            let filteredJob=this.state.totalJobs.filter(fj=>fj.gts_job_application_status=="completed");
            this.setState({CompletedJob:filteredJob})
            console.log("Completed Status are ",this.state.CompletedJob);
        }).catch((err) => {
            console.log("error");
        });
    }
    ...
    render() {
        const {error,CompletedJob}=this.state;
        console.log("in jobs",CompletedJob);
 

        return (this.state.CompletedJob.map(appliedJob=>(
