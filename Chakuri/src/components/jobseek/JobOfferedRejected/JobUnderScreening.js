...
class JobUnderScreening extends Component {
    constructor() {
        super();
        this.state = {
    ...
    componentDidMount(){
        ...
        .then((result) => {
            ...
            let filteredJob=this.state.totalJobs.filter(fj=>fj.gts_job_seeker_status=="Under Screening");
            this.setState({UnderSJob:filteredJob})
            console.log("Completed Status are ",this.state.RejectedJob);
        }).catch((err) => {
            console.log("error");
        });
    }
    ...
    render() {
        const {error,UnderSJob}=this.state;
        console.log("in jobs",UnderSJob);
 

        return (this.state.UnderSJob.map(appliedJob=>(
...
export default JobUnderScreening;
