...
class JobRejected extends Component {
    constructor() {
        super();
        this.state = {
    ...
    componentDidMount(){
        ...
        .then((result) => {
            ...
            let filteredJob=this.state.totalJobs.filter(fj=>fj.gts_job_seeker_status=="Rejected");
            this.setState({RejectedJob:filteredJob})
    ...
    render() {
        const {error,RejectedJob}=this.state;
        console.log("in jobs",RejectedJob);
 

        return (this.state.RejectedJob.map(appliedJob=>(
