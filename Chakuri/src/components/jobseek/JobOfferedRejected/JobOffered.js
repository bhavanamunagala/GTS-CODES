...
class JobOffered extends Component {
    constructor() {
        super();
        this.state = {
    ...
    componentDidMount(){
        ...
        .then((result) => {
            ...
            this.setState({OfferedJob:filteredJob})
            console.log("offered Status are ",this.state.OfferedJob);
        }).catch((err) => {
            console.log("error");
        });
    }
    ...
    render() {
        const {error,OfferedJob}=this.state;
        console.log("in jobs",OfferedJob);
 

        return (this.state.OfferedJob.map(appliedJob=>(
