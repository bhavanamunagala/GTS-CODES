...
class Job extends Component {
    ...
    componentDidMount(){
        ...
        .then((result) => {
            ...
            this.setState({resumeReviewed:filteredJob})
            console.log("reviewed jobs are ",this.state.resumeReviewed);
        }).catch((err) => {
            console.log("error");
        });
    }
    ...
    render() {
        const {error,resumeReviewed}=this.state;
        console.log("in jobs",resumeReviewed);
 

        return (this.state.resumeReviewed.map(appliedJob=>(
