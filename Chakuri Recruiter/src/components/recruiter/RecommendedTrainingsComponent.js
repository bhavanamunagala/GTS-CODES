...
function RecommendedTrainingsComponent(props) {  
  const [data, setData] = useState([]);  

  useEffect(() => {  
    const GetData = async () => {  
      const result = await axios('http://localhost:1202/recommended_trainings');  
      setData(result.data);  
};  
GetData();  
...
export default RecommendedTrainingsComponent;